<template>
  <section class="page-wrap no-padding" @click="showSearch=false">
    <a-page-header :title="$t('livelyData.title')" >
      ...
      <section v-if="!noData" class="content">
        ...
        <section class="chart-block">
          <div class="flex x-space-between">
            <h4>{{ $t('livelyData.active.device.last30days') }}</h4>
        ...
        <section>
          <div class="flex x-space-between">
            <h4>{{ $t('livelyData.active.constitute.last30days') }}</h4>
            <a-button type="link" class="export-btn">{{ $t('public.export') }}</a-button>
          </div>
          <section class="chart-wrap">
...
export default {
  ...
  data() {
    return {
      ...
      totalData:{},
      totalList:[
        { label:this.$t('livelyData.total.today'), key:'today'},
        { label:this.$t('livelyData.total.sevenDays'), key:'sevenDays'},
        { label:this.$t('livelyData.total.total'), key:'total'},
      ],
    };
  },
