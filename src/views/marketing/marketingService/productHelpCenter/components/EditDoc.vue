...
import { getHelpDocDetail, addHelpDoc, updateHelpDoc } from "@/api/productHelpCenter"
...
export default ({
  ...
  watch:{
    visible(val){
      if(!val) return
      this.currentLang = 'zh'
      if(this.relationId) this.getEntryDetail()
  ...
  methods:{
    async getEntryDetail(){
      ...
      const res = await getHelpDocDetail(param)
    ...
    handleOk(){
      this.$refs.form.validate(async(valid) => {
        ...
        const res = this.relationId ? await updateHelpDoc(param) : await addHelpDoc(param)
