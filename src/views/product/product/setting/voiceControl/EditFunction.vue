      ...
      <a-form-model ref="form" class="form" :model="form" :rules="rules" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
        <a-form-model-item :label="$t('setting.voiceControl.editFunction.label.props')" prop="attrCode" >
          <section class="flex y-axis-center">
            ...
            <a-input class="voice-prop-input" v-model="form.voiceCode" :maxLength="100" :placeholder="$t('setting.voiceControl.editFunction.placeholder.props')"/>
          </section>
        </a-form-model-item>
        ...
        <a-form-model-item :label="$t('setting.voiceControl.editFunction.label.dataType')" prop="vDataType" >
          <section class="flex y-axis-center">
            ...
            <span class="map-text">{{$t('setting.voiceControl.editFunction.label.map')}}</span>
            <a-select v-model="form.vDataType" :options="vDataTypeOptions" class="function-select" @change="handleVDataType"/>
          </section>
        </a-form-model-item>
        <a-form-model-item v-if="form.valueMap && form.valueMap.unit" :label="$t('setting.voiceControl.editFunction.unit')" prop="vUnit" >
          ...
          <template v-if="form.vDataType == 'INT' || form.vDataType == 'DOUBLE' ||form.vDataType == 'FLOAT'">
            <span class="map-text">{{$t('setting.voiceControl.editFunction.label.map')}}</span>
            <a-select v-model="form.valueMap.vUnit" :options="voiceUnitList" class="function-select"/>
          </template>
        </a-form-model-item>
        <a-form-model-item v-if="form.valueType == 1 || form.valueType == 2" :label="$t('setting.voiceControl.editFunction.label.value')" prop="valueMap" >
          ...
          <section v-if="form.valueType == 2" class="numerical-list-map">
            <div class="flex y-axis-center value-wrap" v-for="(item,index) in form.valueMap" :key="index">
              ...
              <span class="map-text">{{$t('setting.voiceControl.editFunction.label.map')}}</span>
              <template v-if="form.vDataType=='ENUM' || form.vDataType=='BOOL'">
                <a-textarea class="voice-prop-textarea" v-model="item.voiceValSynonym" :maxLength="500" placeholder="设置别名，多个以半角;隔开" />
              </template>
              <template v-if="form.vDataType=='INT' || form.vDataType=='DOUBLE' || form.vDataType=='FLOAT'">
                <a-input-number class="range-input" v-model="item.vMin" :max="9999999999" :precision="form.vDataType === 'INT'?0:3" :placeholder="$t('setting.voiceControl.editFunction.min')"/>
                <a-input-number class="range-input" v-model="item.vMax" :max="9999999999" :precision="form.vDataType === 'INT'?0:3" :placeholder="$t('setting.voiceControl.editFunction.max')"/>
                <a-input-number class="range-input" v-model="item.vStep" :min="0" :max="9999999999" :precision="form.vDataType === 'INT'?0:3" :placeholder="$t('setting.voiceControl.editFunction.step')"/>
              </template>
...
import { getVoiceUnit } from '@/api/product'
    ...
    return {
      ...
      rules: {
        ...
        attrCode: [
          ...
          { validator:(rule, value, callback) => { !this.form.attrCode || !this.form.voiceCode ? callback(this.$t('setting.voiceControl.editFunction.rules.props')):callback()},  trigger: 'change' }
        ],
        vDataType:[
      ...
      vDataTypeOriginOptions:{
        ...
        'ENUM':[{"label":"整型（Int）","value":"INT"},{"label":"浮点型（Float）","value":"FLOAT"},{"label":"浮点型（Double）","value":"DOUBLE"},{"label":"枚举值（Enum）","value":"ENUM"}],
  ...
  watch:{
    async visible(val){
      ...
      if(this.type == 'edit'){
        ...
        if(this.form.valueMap.unit) this.getVoiceUnit(this.form.valueMap.unit)
  ...
  methods:{
    ...
    handleChangeFun(val){
      ...
      if(this.selectedFun.dataType === 'TEXT'){
        ...
        const data = this.selectedFun?.dataSpecs ? JSON.parse(this.selectedFun.dataSpecs) : {}
        if(data.unit) this.getVoiceUnit(data.unit)
        this.form.vDataType = ''
      } else if (this.selectedFun.dataType === 'ENUM' || this.selectedFun.dataType === 'BOOL'){
        this.updateFuncValueOptions()
        if(this.selectedFun.dataType === 'BOOL'){
          this.form.vDataType = 'BOOL'
        } else {
          this.form.vDataType = ''
        }
      }
    ...
    setValueType(){
      if((this.form.dataType === 'INT' || this.form.dataType === 'DOUBLE' || this.form.dataType === 'FLOAT')&&(this.form.vDataType === 'INT' || this.form.vDataType === 'DOUBLE' || this.form.vDataType === 'FLOAT')){
        ...
        if(this.form.dataType === 'INT' || this.form.dataType === 'DOUBLE' || this.form.dataType === 'FLOAT'){
          ...
          this.form.valueMap = [{ val:'', voiceValSynonym:'' }]
        }
      }
    },
    ...
    // 新增数值
    addMap(){
      this.form.valueMap.push({val:'',voiceValSynonym:''})
    },
    ...
    valueTypeValidator(rule, value, callback){
      if(this.form.valueType === 1){
        ...
        for(let i=0;i<this.form.valueMap.length;i++){
          ...
          if( this.form.vDataType=='INT' || this.form.vDataType=='DOUBLE' || this.form.vDataType=='FLOAT'){
            if(isNull(item.vMin) || isNull(item.vMax) ||isNull(item.vStep)){
              errorTips = this.$t('setting.voiceControl.editFunction.rules.valueType')
              break
            }
          }
    ...
    // 第三方语控平台属性单位列表
    async getVoiceUnit(unit){
      const res = await getVoiceUnit({unit})
      if(res.code !== 0) return
      this.voiceUnitList = res.data?.list?.map(item=>{
        return {
          value: item.unit,
          label: item.label,
        }
