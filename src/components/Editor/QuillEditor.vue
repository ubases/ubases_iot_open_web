...
// 背景颜色 - background, 加粗- bold, 颜色 - color, 字体 - font, 内联代码 - code, 斜体 - italic, 链接 - link, 大小 - size, 删除线 - strike
// 上标/下标 - script, 下划线 - underline, 引用- blockquote, 标题 - header, 缩进 - indent, 列表 - list, 文本对齐 - align, 文本方向 - direction
// 代码块 - code-block, 公式 - formula, 图片 - image, 视频 - video, 清除字体样式- clean
...
export default {
  ...
  props: {
    prefixCls: { type: String, default: 'ant-editor-quill'},
    // 表单校验用字段
          ...
          toolbar: {
            ...
            handlers: {
              ...
              video: function (value) {
                if (value) {
                  document.getElementById("quill-video").value = ''
  ...
  methods: {
    ...
    onChangeVideo() {
      ...
      if (!isType) {
        this.$message.warning(this.$t('QuillEditor.vedio.limit'));
        return;
      }
    ...
    onChangeImg() {
      ...
      if (!isType) {
        this.$message.warning(this.$t('QuillEditor.image.limit'));
        return;
      }
    ...
    async uploadFile(type, file) {
      this.spinning = true
      const res = await uploadFile(file,'editor')
      if (res && res.code == 0) {
        let quill = this.$refs.myQuillEditor.quill;
        let length = quill.selection.savedRange.index //获取插件光标index
